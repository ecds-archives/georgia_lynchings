...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        from georgia_lynchings.demographics.models import County as DCounty
        from georgia_lynchings.lynchings.models import County, Lynching

        for lynching in Lynching.objects.all():
            county = DCounty.objects.get(name=lynching.county.label)
            lynching.d_county = county
            lynching.save()
            for alt_county in lynching.alternate_counties.all():
                county = DCounty.objects.get(name=alt_county.label)
                lynching.d_alt_counties.add(county)
    ...
    def backwards(self, orm):
        ...
        for lynching in Lynching.objects.all():
            lynching.d_alt_counties.clear()
            lynching.d_county = None
            lynching.save()
